...
Mesto_a_venkov %>%
  ...
  ggplot(mapping = aes(x = Kategorie, y = Dostupnost)) + 
  facet_grid(Bydliste~.)+
...
Mesto_a_venkov %>%
  ...
  summarise_at(
    .vars = c("Vzdelavaci_instituce", "Zdravotni_pece", "Kultura", "Moznost_vyberu",
              "Priroda", "Dostupnost_blizkych", "Prace", "Donaska", "Sluzby", "Restaurace"),
    .funs = list(XSoucet = ~sum(.x=="Ano"), XProcento = ~sum(.x=="Ano")/n()*100)
  ) 
# Sprav najprv pivot_longer, ty kaco!
```
...
dostup_data %>%
  ggplot(mapping = aes(x = Kategorie, y = Zastoupeni)) + 
  facet_grid(Bydliste~.) + 
  geom_bar(stat = "identity", position = "dodge", width = 0.5) + 
  theme(
    #legend.position = "none",
